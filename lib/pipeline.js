...
//
// `partial` withholds the last_success_at advance WITHOUT recording an error:
// the nightly budget cap uses it, so a night that processed only its allowance
// re-walks the same window tomorrow and picks up the deferred items (already-
// processed ones dedupe out before any model call, so re-walks cost nothing).
