...
async function toolSearchLeads(newsroomId, args) {
  ...
  return {
    counters: { found: k.found, already_clients: k.suppressed, returned: leads.length, researched },
    shared_opportunity,
    presentation_hint: `Write the way a salesperson talks to a colleague — plain and clear. AVOID jargon words: don't say "buyer", "cohort", "series", "pipeline", "competitive pool", "campaign", "targets", "angle". Say "the organisation that put the work out", "a group of companies", "a set of tenders", "companies to call". Then: (1) if shared_opportunity is present, lead with it in plain words; (2) give the top 3-5 companies a short, readable paragraph each — what they do, why they're worth a call, who to speak to, and what to watch for — using their "why", website and profile; (3) for any top company not yet researched (enriched=false), look it up with your OWN web search (website, offices, size, who to approach, published contacts, why they'd be interested, why they might say no) and call save_profile BEFORE presenting, so the team keeps it; (4) then list the rest briefly. ${researched} of ${leads.length} shown are researched.`,
    leads,
  };
}
