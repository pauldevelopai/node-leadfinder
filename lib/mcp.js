...
import { sweepCompaniesForTenant, CALL_SHEET_FORM } from './companies.js';
...
const TOOLS = [
  ...
  {
    name: 'enrich_company',
    description: 'Research one company on the web NOW and store the profile: website, what they do, offices, headcount, revenue if published (with the honest reason when it is not — private SA companies rarely publish financials), who to approach and any published contacts, why they fit Leads 2 Business, likely objections, and the source URL for every claim. Takes ~30–60 seconds. Facts that cannot be verified come back null with a reason — never invented. The profile is stored, so future questions about this company answer instantly.',
    inputSchema: { type: 'object', properties: {
      company_id: { type: 'string', description: 'UUID from search_leads' },
    }, required: ['company_id'] },
  },
