...
import { sweepCompaniesForTenant, CALL_SHEET_FORM, importClientRows, buildWhy, computeCompanyFields,
         upsertCompany, recordSignal, scoreCompany, ensureCompanyCriteria } from './companies.js';
...
const TOOLS = [
  ...
  {
    name: 'log_outcome',
    description: 'THE FEEDBACK LOOP — the most valuable thing a user can give the system. Log where a lead stands: claimed → called → meeting → converted → lost → retained_90d → retained_12m. ALWAYS ask for and record the reason on "converted" (what made them buy?) and on "lost" (why not — cash flow? already with a competitor? too small?): the reasons are training data. After ~50 decided outcomes LeadFinder compares what it scored high against what actually converted AND STAYED, and proposes reweighted criteria with its reasoning — a human approves before anything changes. When a user mentions a sale or a dead lead in passing, offer to log it.',
    inputSchema: { type: 'object', properties: {
      company_id: { type: 'string' },
      stage: { type: 'string', enum: OUTCOME_STAGES },
      reason: { type: 'string', description: 'WHY — what made it convert, or why it was lost. Ask if not given.' },
      note: { type: 'string' },
    }, required: ['company_id', 'stage'] },
  },
  {
    name: 'add_lead',
    description: 'DISCOVER a new lead the harvest has not surfaced. Workflow: call get_criteria first to see what makes a good L2B lead, then use your OWN web search to find a company that matches (e.g. a contractor in a target CIDB band actively tendering, a firm that just lost a big bid, a company expanding into public work), and add it here with your reasoning. This lets the team find leads beyond the eTenders feed — any province, any source you can search. Provide name (required) and match_reason (required — why it fits the criteria, this is the evidence); optionally province, website, cidb_grading, contacts, sources. A discovered lead is stored with clear provenance (it is NOT grounded in a real tender bid, so it ranks below active bidders until it earns signals) and can then be enriched, claimed and worked like any lead. After adding, call save_profile to attach the full research you did.',
    inputSchema: { type: 'object', properties: {
      name: { type: 'string' },
      match_reason: { type: 'string', description: 'Why this company fits the criteria — the grounding for adding it' },
      province: { type: 'string' },
      website: { type: 'string' },
      cidb_grading: { type: 'string' },
      contacts: { type: 'array', items: { type: 'object' }, description: '[{name,email,phone}]' },
      sources: { type: 'array', items: { type: 'string' }, description: 'URLs backing the match' },
  ...
  {
    name: 'cidb_lookup',
    description: 'Look one company up on the official CIDB Register of Contractors and store what it finds: grading (which sets the territory band — 6+ / 2-5 / 1), registration status, expiry date (a renewing contractor is spending money — a live buying signal), and B-BBEE. FREE — no AI, straight from the public register. A confident single match attaches automatically; an ambiguous result is kept as a "possible match" for a person to confirm, never guessed. Use this to fill the CIDB grade a lead is missing (get_company shows it in the gaps).',
    inputSchema: { type: 'object', properties: {
      company_id: { type: 'string', description: 'UUID from search_leads' },
    }, required: ['company_id'] },
  },
...
async function toolSearchLeads(newsroomId, args) {
  ...
  const { rows } = await pool.query(
    `SELECT c.id, c.name, c.cidb_grading, c.grade_band, c.province, c.total_score, c.band,
            c.contacts, c.enrichment, c.routing_reason, c.current_stage, c.claimed_by_name, c.last_signal_at,
  ...
  // Attach each lead's real reason, built from its signals (free, no AI).
  const buyerTally = {};   // buyer -> count, for the campaign insight
  const leads = [];
  for (const c of rows) {
    const { rows: sigs } = await pool.query(
      `SELECT kind, value, occurred_at, evidence_note, raw FROM leadfinder.company_signals
        WHERE company_id = $1 ORDER BY occurred_at DESC NULLS LAST LIMIT 20`, [c.id]);
    const fields = await computeCompanyFields(c);
    const why = buildWhy(c, sigs, fields);
    for (const b of new Set(sigs.map((s) => s.raw?.buyer).filter(Boolean))) buyerTally[b] = (buyerTally[b] || 0) + 1;
    leads.push({
      id: c.id, name: c.name, province: c.province, band: c.band, total_score: c.total_score,
      cidb_grading: c.cidb_grading, grade_band: c.grade_band,
      website: c.website, enriched: c.enriched,
      claimed_by_name: c.claimed_by_name, current_stage: c.current_stage,
      signals: sigs.length,
      why: why.summary, gaps: why.gaps,
    });
  }
  // Self-describing totals so the client can't mislabel them. "found" was
  // ambiguous and got reported as "149 green, all researched" when the truth
  // was 51 green / 98 amber / 18 researched. Each number is now named and the
  // summary is a ready-to-quote sentence.
  const { rows: [k] } = await pool.query(
    `SELECT COUNT(*) FILTER (WHERE NOT suppressed_as_existing)::int AS total,
            COUNT(*) FILTER (WHERE NOT suppressed_as_existing AND band='green')::int AS green,
            COUNT(*) FILTER (WHERE NOT suppressed_as_existing AND band='amber')::int AS amber,
            COUNT(*) FILTER (WHERE suppressed_as_existing)::int AS already_clients,
            COUNT(*) FILTER (WHERE NOT suppressed_as_existing AND enrichment IS NOT NULL)::int AS researched
       FROM leadfinder.companies WHERE newsroom_id = $1`, [newsroomId]);

  // A shared-opportunity note: when many of these companies bid for the same
  // organisation's work, that's a ready-made reason to call them together —
  // written in plain sales language, not procurement jargon.
  const topBuyer = Object.entries(buyerTally).sort((a, b) => b[1] - a[1])[0];
  const shared_opportunity = topBuyer && topBuyer[1] >= 3
    ? `${topBuyer[1]} of these ${leads.length} companies all put in bids for work from the same organisation (${topBuyer[0]}) and none of them won it. That's a good reason to call all ${topBuyer[1]} with the same friendly opening: they're clearly going after this kind of work and just missed out this time, and LeadFinder would help them spot the next opportunity like it sooner.`
    : null;
  const researched = leads.filter((l) => l.enriched).length;

  const shownResearched = leads.filter((l) => l.enriched).length;

  return {
    counters: {
      companies_on_list_total: k.total,
      green_call_first: k.green,
      amber_worth_a_look: k.amber,
      already_your_clients_hidden: k.already_clients,
      researched_so_far: k.researched,
      shown_in_this_answer: leads.length,
      shown_that_are_researched: shownResearched,
    },
    counts_summary: `${k.total} companies on the list — ${k.green} to call first, ${k.amber} worth a look. ${k.researched} of the ${k.total} have been researched. This answer shows ${leads.length} of them (${shownResearched} researched).`,
    shared_opportunity,
    presentation_hint: `Quote counts_summary as-is if you state numbers — do NOT recompute them or call the whole list "green", and do NOT say "all researched" unless researched_so_far equals companies_on_list_total. Write the way a salesperson talks to a colleague — plain and clear. AVOID jargon: don't say "buyer", "cohort", "series", "pipeline", "competitive pool", "campaign", "targets", "angle", "band". Say "the organisation that put the work out", "a group of companies", "companies to call first". Then: (1) if shared_opportunity is present, lead with it in plain words; (2) give the top 3-5 companies a short, readable paragraph each — what they do, why they're worth a call, who to speak to, and what to watch for — using their "why", website and profile; (3) for any top company not yet researched (enriched=false), look it up with your OWN web search (website, offices, size, who to approach, published contacts, why they'd be interested, why they might say no) and call save_profile BEFORE presenting; (4) then list the rest briefly. If a company's own line of business looks unrelated to construction (a law firm, a security company, an IT supplier), say so plainly and rank it down — a big contract value alone does not make it a good lead for a construction-tender service.`,
    leads,
  };
}
...
async function toolGetCompany(newsroomId, args) {
  ...
  const computed = await computeCompanyFields(row);
  const why = buildWhy(row, signals.rows, computed);
  return {
    ...pub, why: why.summary, gaps: why.gaps,
    scoring_inputs: fields, signals: signals.rows,
    call_sheets: reviews.rows, outcome_history: outcomes.rows,
    presentation_hint: pub.enrichment
      ? 'Present the full profile: website, offices, size, who to approach with contacts, why they fit, likely objections, and the "why". Flag anything marked null/ai_drafted as unverified.'
      : 'This company is not researched yet. Research it now with web search (website, offices, size, who to approach, published contacts, why they would buy, why they might resist), call save_profile to keep it, THEN present the full briefing — do not answer with just the bidding record.',
  };
}

// Discovery: a connected LLM finds a criteria-matching company on the web and
// adds it. Stored with a 'discovery' signal for provenance — it carries no bid
// evidence, so it scores below active bidders until it earns real signals. That
// asymmetry is correct: a hunch grounded in web research is a weaker lead than
// a company we watched bid and lose. Free to the operator — the client's own
// web search does the finding.
async function toolAddLead(newsroomId, args) {
  const name = String(args.name || '').trim();
  if (!name) throw { code: -32602, message: 'name is required.' };
  if (!args.match_reason || !String(args.match_reason).trim()) {
    throw { code: -32602, message: 'match_reason is required — say why this company fits the criteria (that is the grounding for adding it).' };
  }
  const company = await upsertCompany({
    newsroomId, name,
    cidbGrading: args.cidb_grading || null,
    province: args.province || null,
    domain: args.website ? String(args.website).replace(/^https?:\/\//, '').replace(/\/.*$/, '') : null,
    contacts: Array.isArray(args.contacts) ? args.contacts.map((c) => ({ ...c, source: 'discovery' })) : [],
  });
  if (!company) throw { code: -32602, message: 'Could not add — the name did not normalise to anything usable.' };
  await recordSignal(company.id, newsroomId, {
    kind: 'discovery', externalId: `discovery:${Date.now()}`, occurredAt: null,
    evidenceNote: `Discovered via web research: ${String(args.match_reason).slice(0, 500)}`,
    raw: { sources: Array.isArray(args.sources) ? args.sources.slice(0, 10) : [], website: args.website || null },
  });
  const criteria = await ensureCompanyCriteria(newsroomId);
  const scored = await scoreCompany(company, criteria);
  return {
    ok: true, id: company.id, name: company.name,
    created: !!company._created,
    band: scored.band, total_score: scored.total,
    note: `Added${company._created ? '' : ' (already existed — merged)'} with discovery provenance. It ranks below active bidders until it earns tender signals — that is by design. Call save_profile to attach your full research, and enrich/CIDB it like any lead.`,
  };
}
...
const dispatch = {
  ...
  cidb_lookup:         async (nid, args) => (await import('./cidb.js')).cidbLookupById(args.company_id, nid),
...
export function mountMcp(app) {
  ...
  // Express's param decoder BEFORE any handler runs — seen in the wild when
  // someone pastes the masked example URL (lf_········ → %B7%B7…) from the
  // connection guide. Answer that as a clean refusal instead of a 500 stack.
