...
async function toolSearchLeads(newsroomId, args) {
  ...
  // The campaign insight: when many leads share one buyer, that shared field is
  // the headline — a whole competitive pool to work as one campaign, not a list.
  const topBuyer = Object.entries(buyerTally).sort((a, b) => b[1] - a[1])[0];
  const campaign = topBuyer && topBuyer[1] >= 3
    ? `${topBuyer[1]} of these ${leads.length} leads bid on the same buyer (${topBuyer[0]}) — a single competitive pool. Lead with that: they bid on the same work and none won, so pitch them as one campaign ("you bid on this series and it didn't land — here's how you'd see the next round earlier").`
    : null;
  ...
  return {
    ...
    presentation_hint: `DO NOT reduce this to one line per company — that is the failure mode. Present it as a briefing: (1) open with the campaign_insight if present; (2) give the top 3-5 leads a real paragraph each using their "why", website and profile; (3) for any of those top leads not yet researched (enriched=false), research it with your OWN web search — website, offices, size, who to approach, published contacts, why they'd buy, why they'd resist — and call save_profile BEFORE presenting, so the team keeps it; (4) only then list the remainder briefly. ${researched} of ${leads.length} shown are researched — research the rest of the top ones now rather than presenting them as bare rows.`,
    leads,
  };
}
