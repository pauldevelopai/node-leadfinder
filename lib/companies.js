...
// ── is this actually a CONSTRUCTION company? ─────────────────────────────────
// The problem this solves, measured 2026-08-19: a law firm (Seanego), a security
// company (Prime African) and an IT supplier (Computerized Technology) were
// topping L2B's call list, because winning a big PUBLIC contract scores well and
// nothing asked what the company does. L2B sells construction-tender
// intelligence, so a non-contractor is not a lead however much it won.
//
// CIDB registration alone does NOT settle it — grade 1-2 with one or two classes
// is cheap and registered opportunistically (Prime African Security holds
// "PE 2GB, 2EP"). What separates a real contractor is the SHAPE of the grading:
// several works classes at a meaningful grade (Goldwave: 12 classes at grade 6;
// Themolo: grades 6-8). So: CIDB shape first, company name second, honest
// uncertainty last. Returns 0..1 with the reason, and never a hard exclusion —
// a human still sees the lead, it simply ranks where it belongs.
const CORE_WORKS_CLASSES = new Set(['CE', 'GB', 'ME', 'EB', 'EP']);   // civil, building, mechanical, electrical
// Words that name a DIFFERENT line of business. Deliberately excludes SA
// construction-neutral words ("trading", "projects", "enterprise",
// "developments") — RNR Trading Solutions is a real contractor.
const OTHER_SECTOR_WORDS = [
  'attorney', 'advocate', 'law firm', 'legal practi', 'security', 'guarding', 'protection services',
  'technology solution', 'computer', 'software', 'it solution', 'stationery', 'printing', 'courier',
  'catering', 'cleaning', 'laundry', 'medical', 'pharmac', 'health care', 'healthcare', 'travel',
  'insurance', 'laboratory', 'recruitment', 'labour broker', 'media', 'marketing', 'advertising',
  'funeral', 'auditor', 'accounting', 'training academy', 'business development and consulting',
];
const CONSTRUCTION_WORDS = [
  'construction', 'civil', 'building', 'builders', 'contracting', 'contractors', 'plant hire',
  'earthworks', 'roads', 'paving', 'plumbing', 'electrical', 'roofing', 'concrete', 'engineering',
  'infrastructure', 'housing', 'developers',
];

export function sectorFit(company) {
  const name = String(company.name || '').toLowerCase();
  const grading = String(company.cidb_grading || '');
  const classes = [...grading.matchAll(/\b([1-9])\s*([A-Z]{2})\b/gi)]
    .map((m) => ({ level: parseInt(m[1], 10), cls: m[2].toUpperCase() }));
  const maxLevel = classes.length ? Math.max(...classes.map((c) => c.level)) : null;
  const coreCount = classes.filter((c) => CORE_WORKS_CLASSES.has(c.cls)).length;
  const nameSaysOther = OTHER_SECTOR_WORDS.some((w) => name.includes(w));
  const nameSaysConstruction = CONSTRUCTION_WORDS.some((w) => name.includes(w));

  // 1. CIDB shape — real capacity settles it, whatever the name says.
  if (classes.length >= 3 && maxLevel >= 3) {
    return { fit: 1, reason: `Registered contractor with real capacity — ${classes.length} CIDB works classes up to grade ${maxLevel}.` };
  }
  if (coreCount >= 1 && maxLevel >= 3) {
    return { fit: 0.9, reason: `CIDB-registered contractor, grade ${maxLevel} in core works classes.` };
  }
  // 2. Minimally registered — cheap to hold, so the name decides.
  if (classes.length) {
    if (nameSaysOther) {
      return { fit: 0.1, reason: `Looks like a different line of business (its name says so) with only a minimal CIDB registration (${grading}) — a big public contract does not make it a construction lead.` };
    }
    if (nameSaysConstruction) {
      return { fit: 0.7, reason: `Construction by name, CIDB-registered but small (${grading}).` };
    }
    return { fit: 0.45, reason: `Minimal CIDB registration (${grading}) and the name does not say what they do — worth a check before calling.` };
  }
  // 3. Not in the register at all.
  if (nameSaysOther) {
    return { fit: 0.05, reason: 'Not a construction company as far as we can tell — its name points to another line of business and it is not on the CIDB register.' };
  }
  if (nameSaysConstruction) {
    return { fit: 0.5, reason: 'Construction by name but not found on the CIDB register — worth confirming they are registered.' };
  }
  return { fit: 0.3, reason: 'We cannot tell what this company does yet — not on the CIDB register and the name gives nothing away.' };
}
...
export const STARTER_COMPANY_CRITERIA = {
  ...
  weights: [
    // Weighted highest on purpose: a company that isn't a construction
    // contractor is not a lead for a construction-tender service, however
    // much public work it wins. Scores linearly with sector_fit (0..1).
    { component: 'sector_fit', weight: 3.5, source: 'prior',
...
// ── the "why" — a real reason, built from data, for free ────────────────────
// The signals already contain the story; nobody was telling it. This turns a
// company's raw signals into the sentence a salesperson actually needs — no AI,
// no cost, instant. It states what they bid on/won, whether they're losing
// (the strongest buy signal for tender intelligence), the competitive cluster,
// and — honestly — what we don't yet know and how to fill it.
export function buildWhy(company, signals = [], fields = null) {
  const bids = signals.filter((s) => s.kind === 'tenderer');
  const awards = signals.filter((s) => s.kind === 'award');
  const buyers = [...new Set(signals.map((s) => s.raw?.buyer).filter(Boolean))];
  const lost = fields?.lost_bid_count_180d;
  const awardValue = fields?.award_value_180d;

  const parts = [];
  if (bids.length) {
    const where = company.province ? ` in ${company.province}` : '';
    const via = buyers.length === 1 ? ` (via ${buyers[0]})` : buyers.length > 1 ? ` across ${buyers.length} buyers` : '';
    parts.push(`Bid on ${bids.length} public tender${bids.length === 1 ? '' : 's'}${where}${via} in the last 6 months.`);
  }
  if (awards.length) {
    const v = awardValue ? ` worth about R${Number(awardValue).toLocaleString('en-ZA')}` : '';
    parts.push(`Won ${awards.length} of them${v} — a spending, active contractor.`);
  } else if (bids.length) {
    parts.push(`No wins recorded against those bids — actively tendering and not yet winning, which is exactly the gap L2B's tender intelligence fills.`);
  }
  if (company.cidb_grading) parts.push(`CIDB ${company.cidb_grading}.`);
  // Say plainly when this doesn't look like a construction company — a caller
  // should never discover that on the phone.
  if (fields?.sector_reason && (fields.sector_fit ?? 1) < 0.6) parts.push(fields.sector_reason);

  // What's missing, named — so the reader knows to research, not that it's empty.
  const gaps = [];
  if (!company.cidb_grading) gaps.push('CIDB grade');
  if (!(Array.isArray(company.contacts) && company.contacts.length)) gaps.push('contacts');
  if (!company.enrichment) gaps.push('company profile (size, offices, who to approach)');
  const gapNote = gaps.length
    ? `Not yet known: ${gaps.join(', ')} — research this company to fill it in.`
    : null;

  return { summary: parts.join(' '), gaps: gapNote, tenders: bids.map((b) => b.evidence_note).filter(Boolean).slice(0, 6) };
}
...
export async function computeCompanyFields(company) {
  ...
  const sector = sectorFit(company);
  return {
    ...
    // Is this a construction company at all? (see sectorFit — the fix for law
    // firms and security companies topping a construction-tender call list.)
...
// A company we don't believe is a construction contractor must never land in
// "call first", however much public work it won — a big contract value was
// carrying a training company (sector_fit 0.05) into green on total score
// alone. The engine's hard_rules only fire on an exact 0, so this is an
// explicit, recorded cap: the arithmetic is untouched, the demotion and its
// reason are persisted in routing_reason so any ranking stays auditable.
const SECTOR_FIT_GREEN_FLOOR = 0.3;
...
export async function scoreCompany(company, criteria) {
  ...
  if (result.band === 'green' && (fields.sector_fit ?? 1) < SECTOR_FIT_GREEN_FLOOR) {
    result.band = 'amber';
    result.routing_reason = `${result.routing_reason} — but held back from "call first": ${fields.sector_reason}`;
  }
...
export async function sweepCompaniesForTenant(newsroomId) {
  ...
  // Enrichment Tier 0: CIDB register lookups for companies missing a grading —
  // free (no model call), per-company only while the bulk-licence question to
  // CIDB stays open, capped + delayed (CIDB_PER_SWEEP / CIDB_DELAY_MS). Runs
  // BEFORE scoring so a grade filled tonight feeds tonight's has_cidb. Lazy
  // import + try/catch for the same reason as the Tier 2 tail below: the
  // sweep must never die on enrichment.
  try {
    const { cidbSweepForTenant } = await import('./cidb.js');
    const cd = await cidbSweepForTenant(newsroomId);
    digest.cidb = { attempted: cd.attempted, matched: cd.matched, possible: cd.possible, none: cd.none };
    digest.notes.push(...cd.notes);
    if (cd.attempted) {
      digest.notes.push(`CIDB register: ${cd.matched} matched, ${cd.possible} possible (queued for a person), ${cd.none} not found — of ${cd.attempted} looked up.`);
    }
  } catch (e) {
    digest.notes.push(`CIDB lookups skipped: ${e.message}`);
  }
