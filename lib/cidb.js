...
function parseShell(html) {
  ...
  // The portal quotes attributes inconsistently: data-get-url is double-quoted
  // but data-view-layouts is SINGLE-quoted (the base64 blob has no quotes inside,
  // so single-quoting is safe for it). Match either style via a backreference to
  // the opening quote — matching only double quotes silently dropped
  // data-view-layouts and produced "grid contract attributes missing".
  const attr = (name) => {
