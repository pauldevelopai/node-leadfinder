...
const PER_SWEEP = parseInt(process.env.ENRICH_PER_SWEEP, 10) || 5;
...
// The sweep's tail: enrich the best unenriched green-band companies, capped.
  ...
  const { rows } = await pool.query(
    `SELECT id, name FROM leadfinder.companies
      WHERE newsroom_id = $1 AND suppressed_as_existing = false
        AND band = 'green' AND enriched_at IS NULL
      ORDER BY total_score DESC NULLS LAST LIMIT $2`, [newsroomId, cap]);
